...
void TL_toReal(float P1, float P2, float F1, float F2, float &xt, float &yt, float &zt){
	float oldz = zt;
	float newz = P1 + 1.f / (1.f / (P2 - F2) - 1.f / (P2 - oldz));
	float scale = (newz - P1) / (oldz - P2);
...
: Camera(cam2world, sopen, sclose, f) // pbrt-v2 doesnot specify hither and yon
{
	...
	for(int i = 1; i <= nlens; i++){
		...
		fprintf(stderr, "lens %d: r:%f axp:%f IoR:%f aper:%f cent:%f zdisp:%f\n", i,
				radius[i], axpos[i], lensN[i], aperture[i], zcenter[i], zbound[i]);
	}
	...
	if(ThickLens(1, nlens, P1, P2, F1, F2) == 0)
		fprintf(stderr, "FAIL\n");
...
float RealisticCamera::GenerateRay(const CameraSample &sample, Ray *ray) const {
	...
	float x = Pstart.x, y = Pstart.y, z = Pstart.z;
	TL_toReal(P1, P2, F1, F2, x, y, z);

	Point Pnext = Incident((P2 - Incident.o.z) / Incident.d.z);
	Pnext.z = P1;

	Vector Truedir = Point(x, y, z) - Pnext;
	Incident = Ray(Pnext, Normalize(Truedir), 0);
